...
export default function BrandMark({
  ...
  return (
    <span className={`brandmark-root inline-flex max-w-full min-w-0 items-center ${gapClassName} ${className}`.trim()}>
      ...
      {withCopy ? (
        <span className="brandmark-copy min-w-0 flex-1">
          <span className="brandmark-copy-title block font-[family:var(--font-display)] text-sm font-semibold uppercase tracking-[0.24em] text-white/90">
            Analysis Studio
          </span>
          {withTagline ? (
            <span className="brandmark-copy-tagline block text-xs text-white/55">
